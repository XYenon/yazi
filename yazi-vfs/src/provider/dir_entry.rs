...
pub enum DirEntry {
	...
	Opendal(super::opendal::DirEntry),
}
...
impl FileHolder for DirEntry {
	async fn file_type(&self) -> io::Result<ChaType> {
		match self {
			...
			Self::Opendal(entry) => entry.file_type().await,
		}
	}
	...
	async fn metadata(&self) -> io::Result<Cha> {
		match self {
			...
			Self::Opendal(entry) => entry.metadata().await,
		}
	}
