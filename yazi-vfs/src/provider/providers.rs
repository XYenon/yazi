...
impl<'a> Provider for Providers<'a> {
	...
	async fn create_new(&self) -> io::Result<Self::File> {
		Ok(match self {
			...
			Self::OpenDal(p) => p.create_new().await?,
		})
	}
	...
	async fn new<'b>(url: Url<'b>) -> io::Result<Self::Me<'b>> {
		...
		Ok(match url.kind() {
			...
			K::OpenDal => Self::Me::OpenDal(super::opendal::OpenDal::from_url(url).await?),
		})
	}
